...
export const generatePix = async (
  pixInfos: RequestGeneratePix,
): Promise<ResponseGeneratedPix> => {
  try {
...
export const verifyPixStatus = async (
  paymentID: string,
): Promise<ResponseVerifyPixStatus> => {
  try {
    const response = await axios.get(
      `${CIELO_QUERY_API_URL}/1/sales/${paymentID}`,
      headers,
    );
...
export const devolutionPix = async (
  paymentID: string,
  amount: number,
): Promise<ResponseDevolutionPix> => {
  try {
    const { data } = await axios<ResponseDevolutionPix>({

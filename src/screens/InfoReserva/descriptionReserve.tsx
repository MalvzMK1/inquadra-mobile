		...
		<View className='flex-1 bg-zinc-600'>
			...
			<View className=' h-16 w-max  bg-zinc-900 flex-row item-center justify-between px-5'>
				...
				<View className='h-max w-max flex justify-center items-center'>
					<TouchableOpacity className='h-12 W-12 ' onPress={() => navigation.navigate('ProfileSettings', {
						userID: user_id,
						userPhoto: dataUser?.usersPermissionsUser.data?.attributes.photo.data?.attributes.url ?? undefined
					})}>
						<Image
				...
				<View className='flex w-max h-fit bg-zinc-900 px-5 pb-2'>
					...
					</View>
					{
						(data && data.scheduling.data && data.scheduling.data.attributes.court_availability.data && serviceRate)
							...
							<>
								{
									(data && data.scheduling.data && data.scheduling.data.attributes.court_availability.data)
									&&
									<View style={{ width: '100%', justifyContent: 'center' }} className='relative'>
										<Text className='absolute z-10 self-center text-white font-bold'>
											Pagamento efetuado
			...
			<View className="absolute bottom-0 left-0 right-0">
				<BottomBlackMenu
					...
					userID={user_id}
					userPhoto={dataUser?.usersPermissionsUser?.data?.attributes?.photo?.data?.attributes?.url ? HOST_API + dataUser?.usersPermissionsUser?.data?.attributes?.photo?.data?.attributes?.url : null}
					key={1}
					isDisabled={true}
					paddingTop={2}
				/>

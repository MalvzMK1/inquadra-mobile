...
export default function HomeEstablishment({
  ...
  const [courtAvailabilityDisponible, setCourtAvailabilityDisponible] = useState<boolean>(false)
  ...
  const { data: dataEstablishmentId } = useGetUserEstablishmentInfos(
    userId || "",
    {
      onCompleted(data) {
        if (
          ...
          data.usersPermissionsUser.data.attributes.establishment.data
        ) {
          setEstablishmentId(
            data.usersPermissionsUser.data.attributes.establishment.data.id,
          );
          setPhoto(
            data.usersPermissionsUser.data.attributes.establishment.data
              ?.attributes.logo.data?.attributes.url ?? undefined,
          );
          setUserName(data.usersPermissionsUser.data.attributes.username);
          setFirstName(
            data.usersPermissionsUser.data.attributes.username.split(" ")[0],
          );
        }
      },
    },
  ...
  const [
    ...
    { error: errorActivateStatus, loading: loadingActivateStatus },
  ...
  useEffect(() => {
    if (
      ...
      dataSchedulings.establishment.data.attributes.courts.data.map((courts) => {
        if (
          courts.attributes.court_availabilities.data.length > 0
        ) {
          ...
          <View className="p-5 flex flex-col justify-between">
              ...
              <View className="pt-5 gap-2">
                {loadingCourtsEstablishment ? (
                  ...
                  <Fragment>
                    ...
                    {handleDayUse === false
                      ? establishmentCourts.map(court =>
                          court?.attributes.court_availabilities.data.map(
                            availabilities =>
            ...
            <View className="pt-10">
              <View className="flex flex-row">
                ...
                <View className="ml-auto flex flex-col items-start">
                  <SelectList
                    setSelected={(value: any) => setFantasyName(value)}
                    data={
            ...
            <View className="flex flex-row flex-1">
              {loadingCourtsEstablishment ? (
                ...
                <View className="gap-4 pt-3 flex-1">
                  {establishmentCourts.length === 0 && (
                    <Text className="text-center">Não há reservas...</Text>
                  )}
                  ...
                  {handleDayUse !== true
                    ? establishmentCourts.map(courts =>
                    ...
                    : <View className="h-max w-full flex justify-center items-center px-2">
                      <Text className="text-base font-bold text-center">Não existem disponibilidades para data atual</Text>
                    </View>
                }
                </View>
              )}

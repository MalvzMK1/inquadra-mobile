...
import parseBRDateToEUADate from "../../utils/parseBRDateToEUADate";
...
export default function CourtSchedule({
	...
	const [blockAvailability] = useBlockAvailability();
	...
	function transformDateStringAndHourIntoDate({date, hour}: { date: string, hour: string }): Date {
		const newDate = new Date(`${date}T${hour}:00.000Z`);

		return newDate;
	}

	function handleBlockSchedule(): void {
		const startsDate = parseBRDateToEUADate(startsAt);
		const endsDate = parseBRDateToEUADate(endsAt);
		const selectedCourtsIds = activeCourts.map(court => court.id);

		const startsDateTime = transformDateStringAndHourIntoDate({
			date: startsDate,
			hour: startHour,
		})

		const endsDateTime = transformDateStringAndHourIntoDate({
			date: endsDate,
			hour: endHour,
		})

...
export default function FavoriteCourts({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'FavoriteCourts'>) {
	...
	const { data, error, loading } = useGetFavoritesCourtsById('1')
	const [color, setColor] = useState({});
	const { data: userByIdData, error: userByIdError, loading: userByIdLoading } = useGetUserById(route.params.userID)

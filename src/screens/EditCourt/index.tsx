...
const courtFormSchema = z.object({
  fantasyName: z.string().nonempty("Insira um nome!"),
...
export default function EditCourt({
  ...
  const {
    data: courtByIdData,
    error: courtByIdError,
    loading: courtByIdLoading,
  } = useCourtById(courtId ?? "");
  const [updateCourtHook, { data, loading, error }] = useUpdateCourt();

  let courtTypesData: string[] = [];
  sportTypesData?.courtTypes.data.map(sportItem =>
  ...
  sportTypesData?.courtTypes.data.map(sportTypeItem => {
    allCourtTypesJson = [
      ...allCourtTypesJson,
      { id: sportTypeItem.id, name: sportTypeItem.attributes.name },
    ];
  });

  const fantasyName = courtByIdData?.court.data.attributes.fantasy_name;
  const minimumScheduleValue: number | undefined =
    courtByIdData?.court.data.attributes.minimumScheduleValue;
  const [minimumScheduleValueState, setMinimumSheduleValue] =
    useState<number>();
  useEffect(() => {
  ...
  if (
    courtByIdData?.court.data.attributes.court_availibilites != null ||
    courtByIdData?.court.data.attributes.court_availibilites != undefined
  ) {
    courtByIdData?.court.data.attributes.court_availibilites.data.map(
      courtAvailibilityItem =>
        courtAvailibilites.push(courtAvailibilityItem.attributes.id),
    );
  }
  ...
  if (
    ...
    courtByIdData?.court.data.attributes.court_types.data.map(courtTypeItem =>
      courtTypes.push(courtTypeItem.id),
    );
  }
  ...
  courtByIdData?.court.data.attributes.court_types.data.map(courtTypeItem => {
    courtTypesJson = [
      ...courtTypesJson,
      { id: courtTypeItem.id, name: courtTypeItem.attributes.name },
    ];
  });
  ...
  const handleUpdateCourt = (data: ICourtFormData): void => {
    ...
    if (courtTypeSelected) {
      courtTypeSelected?.map(courtTypeSelectedItem => {
        ...
        if (courtTypeIdItem != undefined) courtTypesId?.push(courtTypeIdItem);
      });
    }
    ...
    <View className="h-full w-full flex flex-col">
      ...
      <View className={`absolute bottom-0 left-0 right-0`}>
        <BottomBlackMenuEstablishment
          ...
          userID={userId}
          establishmentLogo={
            route.params.userPhoto !== undefined ||
            route.params.userPhoto !== null
              ? HOST_API + route.params.userPhoto
              : null
          }
          establishmentID={

...
export default function InfoProfileEstablishment({
  ...
  const [selected, setSelected] = useState<
    EEstablishmentEditableData | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (
      ...
      navigation.setParams({
        establishmentPhoto:
          userByEstablishmentData.usersPermissionsUser.data.attributes
            .establishment.data.attributes.logo.data?.attributes.url ??
          undefined,
        establishmentId:
          userByEstablishmentData.usersPermissionsUser.data.attributes
            .establishment.data.id,
  ...
  const [updateCorporateNameLoading, setUpdateCorporateNameLoading] =
    useState<boolean>(false);

  function handleUpdateEstablishmentCorporateName(
    value: ICorporateNameFormData,
  ): void {
    try {
      ...
      updateEstablishmentFantasyNameHook({
        variables: {
          establishment_id: establishmentId ? String(establishmentId) : "",
        ...
        .then(response => {
          ...
          throw new Error(
            "A razão social cadastrada não condiz com a do banco de dados",
          );
        })
        .catch(reason => {
  ...
  return (
    <View className="flex-1 bg-white h-full">
      <ScrollView className="flex-grow p-1">
          ...
          {showCard &&
            (loadingPayment ? (
              ...
              <View>
                          ...
                          onPress={() => {
                            deletePixKey({
                              ...
                              .then(response => {
                                if (
                                  response.data &&
                                  response.data.deletePixKey.data.id === card.id
                                ) {
                                  alert("Chave Pix deletada com sucesso");
                                  const pixKeyIndexInArray =
                                    withdrawalInfo.findIndex(
                                      info => info.id === card.id,
                                    );

                                  if (pixKeyIndexInArray >= 0) {
                                    const newWithdrawalInfo =
                                      withdrawalInfo.filter(
                                        info => info.id !== card.id,
                                      );

                                    setWithdrawalInfo(newWithdrawalInfo);
                                  }
                                }
                              })
                              .catch(reason => {
                                console.error(JSON.stringify(reason, null, 2));
                                alert("Não foi possível deletar a chave pix");
                              });
                          }}
                        >
                          <Ionicons
                ...
                />
                <View className="border border-gray-500 p-4 mt-2 rounded-lg">
        ...
        >
          <View className="h-full w-full justify-center items-center bg-black/70">
        ...
        >
          <View className="h-full w-full justify-center items-center bg-black/70">
            <View className="h-fit w-[350px] bg-[#f8f4f2] rounded-[5px] p-6">
              ...
              />
              {errorsCorporateName.corporateName && (
                <Text className="text-red-400 text-sm -pt-[10px]">
                  {errorsCorporateName.corporateName.message}
                </Text>
              )}
              <View className="flex flex-row w-full justify-between pt-2">
                ...
                <TouchableOpacity
                  onPress={handleSubmitCorporateName(
                    handleUpdateEstablishmentCorporateName,
                  )}
                  className="h-fit w-[146px] rounded-md bg-[#FF6112] flex items-center justify-center ml-[4px] p-[8px]"
                >
                  <Text className="text-white font-medium text-[14px]">

													...
													onPress={() => {
														deletePixKey({
															...
															if (
																...
																const pixKeyIndexInArray = withdrawalInfo.findIndex(info => info.id === card.id);

																if (pixKeyIndexInArray >= 0) {
																	const newWithdrawalInfo = withdrawalInfo.filter(info => info.id !== card.id);
					...
					<View className="h-full w-full justify-center items-center bg-[#3339]">
						<View className="h-fit w-[350px] bg-[#f8f4f2] rounded-[5px] p-6">
							...
							/>
							{
								...
								<Text className="text-red-400 text-sm -pt-[10px]">{errorsCorporateName.corporateName.message}</Text>
							}

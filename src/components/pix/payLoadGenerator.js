...
var payload = function (pixKey, description, merchantName, merchantCity, txId, value) {
    ...
    var descriptionValue = description === '' || description === null ? '' : getValue(ID_MERCHANT_ACCOUNT_INFORMATION_DESCRIPTION, description);
    var accountValues = getValue(ID_MERCHANT_ACCOUNT_INFORMATION, "".concat(gui).concat(key).concat(descriptionValue));
    ...
    var payload = "".concat(getValue(ID_PAYLOAD_FORMAT_INDICATOR, '01')).concat(accountValues).concat(getValue(ID_MERCHANT_CATEGORY_CODE, '0000')).concat(getValue(ID_TRANSACTION_CURRENCY, '986')).concat(getValue(ID_TRANSACTION_AMOUNT, value)).concat(getValue(ID_COUNTRY_CODE, 'BR')).concat(getValue(ID_MERCHANT_NAME, merchantName)).concat(getValue(ID_MERCHANT_CITY, merchantCity)).concat(additionalIdField);

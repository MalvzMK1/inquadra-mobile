...
export function UserProvider({ children }: IUserProviderProps) {
  ...
  async function handleSetUserData(
    ...
    if (props !== undefined) {
      ...
      await AsyncStorage.removeItem("@inquadra/user_data");
    }
  ...
  useEffect(() => {
    async function loadDataFromAsyncStorage(): Promise<{
      newUserData: IUserData | undefined;
      newUserGeolocation: UserGeolocation | undefined;
    }> {
      ...
      if (loadedUserData) {
        try {
          ...
          setUserData(parsedLoadedUserData);
          newUserData = parsedLoadedUserData;
        } catch (error) {
          console.log(JSON.stringify(error));

          if (JSON.parse(APP_DEBUG_VERBOSE)) {
            alert(JSON.stringify(error, null, 2));
          } else {
            alert("Não foi possível guardar as informações do usuário");
          }
        }
      }
